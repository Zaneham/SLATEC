#!/usr/bin/env python3
"""Fix GOTO in dhstrt.f90 - DEPAC starting step size (double precision)"""

filepath = 'C:/dev/slatec-modern/src/modern/diff_integ_eq/depac/dhstrt.f90'
...
# Line 269: IF( k==lk ) GOTO 100
old_goto = '''    !     ......EXIT
    IF( k==lk ) GOTO 100'''
new_goto = '''    !     Exit when k==lk (last iteration)
    IF( k==lk ) EXIT'''

content = content.replace(old_goto, new_goto)

# Remove label 100 (code after loop continues normally)
old_label = '''  !
...
new_label = '''  !
  ...
  !
  !          COMPUTE A BOUND (YDPB) ON THE NORM OF THE SECOND DERIVATIVE
  !          (Label 100 removed - reached via EXIT or loop completion)
  !
...
print('dhstrt.f90: GOTO 100 eliminated (skip-to-end -> EXIT)!')
