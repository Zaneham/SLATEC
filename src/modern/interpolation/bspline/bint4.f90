...
PURE SUBROUTINE BINT4(X,Y,Ndata,Ibcl,Ibcr,Fbcl,Fbcr,Kntopt,T,Bcoef,N,K,W)
  ...
  ELSE
    ndm = Ndata - 1
    DO i = 1, ndm
      IF( X(i)>=X(i+1) ) ERROR STOP 'BINT4 : X VALUES ARE NOT DISTINCT OR NOT ORDERED'
    END DO
      ...
      SELECT CASE (Kntopt)
        ...
        CASE (3)
          ...
          DO i = 1, 3
            ...
            iw = MOD(i+2,5) + 1
            T(np+i) = W(iw,jw)
            IF( T(4-i)>T(5-i) ) ERROR STOP 'BINT4 : KNOT INPUT THROUGH W ARRAY IS NOT ORDERED PROPERLY'
            IF( T(np+i)<T(np+i-1) ) ERROR STOP 'BINT4 : KNOT INPUT THROUGH W ARRAY IS NOT ORDERED PROPERLY'
          END DO
        CASE DEFAULT
