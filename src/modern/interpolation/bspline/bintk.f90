...
PURE SUBROUTINE BINTK(X,Y,T,N,K,Bcoef,Q,Work)
  ...
  ELSE
    ...
    ELSE
      ...
      IF( jj/=0 ) THEN
        DO i = 1, jj
          IF( X(i)>=X(i+1) ) ERROR STOP 'BINTK : X(I) DOES NOT SATISFY X(I)<X(I+1) FOR SOME I'
        END DO
      END IF
      np1 = N + 1
      km1 = K - 1
      ...
      DO i = 1, N
        ...
        left = MAX(left,i)
        IF( xi<T(left) ) ERROR STOP 'BINTK : SOME ABSCISSA WAS NOT IN THE SUPPORT OF THE&
        ...
        DO WHILE( xi>=T(left+1) )
          ...
          IF( left>=ilp1mx ) THEN
            left = left - 1
            IF( xi<=T(left+1) ) EXIT
            ERROR STOP 'BINTK : SOME ABSCISSA WAS NOT IN THE SUPPORT OF THE&
              & CORRESPONDING BASIS FUNCTION AND THE SYSTEM IS SINGULAR.'  ! (was GOTO 20)
          END IF
        END DO
