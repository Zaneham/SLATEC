...
PURE SUBROUTINE DAVINT(X,Y,N,Xlo,Xup,Ans,Ierr)
  ...
  !   251216  Eliminated GOTO 50 per MODERNISATION_GUIDE.md §1. (ZH)
  !           Ref: ISO/IEC 1539-1:2018 §11.1.8 (IF-THEN-ELSE)
  ...
  ELSEIF( Xlo/=Xup ) THEN
    IF( N>=2 ) THEN
      DO i = 2, N
        IF( X(i)<=X(i-1) ) THEN
          ! Error: abscissas not strictly increasing (IERR=4)
          ! Ref: SC-M-69-335 (Jones, 1969)
          Ierr = 4
