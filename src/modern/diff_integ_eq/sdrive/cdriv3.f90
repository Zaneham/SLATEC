...
SUBROUTINE CDRIV3(N,T,Y,F,Nstate,Tout,Ntask,Nroot,Eps,Ewt,Ierror,Mint,&
  ...
  ELSEIF( (Impl>=1 .AND. Impl<=3) .AND. Miter==0 ) THEN
    WRITE (intgr1,'(I8)') Impl
    Ierflg = 30
    ERROR STOP 'CDRIV3 : Illegal input.  For MITER = 0, the value of IMPL is not allowed.'
    Nstate = 12
    RETURN
  ELSEIF( (Impl==2 .OR. Impl==3) .AND. Mint==1 ) THEN
    WRITE (intgr1,'(I8)') Impl
    Ierflg = 31
    ERROR STOP 'CDRIV3 : Illegal input.  For MINT = 1, the value of IMPL is not allowed.'
    Nstate = 12
    RETURN
  END IF
  ...
  IF( Leniw<liwchk ) THEN
    ...
    Ierflg = 33
    ERROR STOP 'CDRIV3 : Illegal input.  Insufficient storage&
  ...
  IF( .NOT. skip_to_main_loop ) THEN
    ...
    IF( Iwork(IJTASK)/=0 ) THEN
      ...
      IF( .NOT. (do_return .OR. do_return_500 .OR. fatal_error) ) THEN
        ...
        IF( Hmax/=REAL(Work(IHMAX)) ) THEN
          h = SIGN(MIN(ABS(h),Hmax),h)
          IF( h/=REAL(Work(IH)) ) THEN
            Iwork(IJTASK) = -1
            Work(IH) = h
          END IF
          Work(IHMAX) = Hmax
  ...
  main_loop: DO
    ...
    SELECT CASE (jstate)
      ...
      CASE (3)
        WRITE (rl1,'(E16.8)') T
        Ierflg = 42
        ERROR STOP 'CDRIV3 : At T the step size has been reduced about 50 &
