...
PURE SUBROUTINE DDASSL(RES,Neq,T,Y,Yprime,Tout,Info,Rtol,Atol,Idid,Rwork,Lrw,&
  ...
  IF( Info(1)==0 ) THEN
    ...
    DO i = 2, 11
      IF( Info(i)/=0 .AND. Info(i)/=1 ) THEN
        ERROR STOP 'DDASSL : SOME ELEMENT OF INFO VECTOR IS NOT ZERO OR ONE'
        illegal_input = .TRUE.
        EXIT
      END IF
    END DO
  ...
  DO i = 1, Neq
    ...
    IF( rtoli<0._DP ) THEN
      ERROR STOP 'DDASSL : SOME ELEMENT OF RTOL IS < 0'
      illegal_input = .TRUE.
      EXIT
    END IF
    IF( atoli<0._DP ) THEN
      ERROR STOP 'DDASSL : SOME ELEMENT OF ATOL IS < 0'
      illegal_input = .TRUE.
      EXIT
    END IF
  END DO
  ...
  IF( Info(1)==1 ) THEN
    ...
    IF( Info(4)==1 ) THEN
      tstop = Rwork(LTSTOP)
      IF( (tn-tstop)*h>0._DP ) THEN
        WRITE (xern3,'(1P,D15.6)') tstop
        WRITE (xern4,'(1P,D15.6)') T
  ...
  ELSE
    ...
    DO i = 1, Neq
      IF( Rwork(lwt+i-1)<=0._DP ) THEN
        ERROR STOP 'DDASSL : SOME ELEMENT OF WT IS <= 0.0'
        Idid = -33
        IF( Info(1)==-1 ) ERROR STOP 'DDASSL : REPEATED OCCURRENCES OF ILLEGAL INPUT&
          & RUN TERMINATED. APPARENT INFINITE LOOP'
        Info(1) = -1
  ...
  integration_loop: DO
    ...
    ELSEIF( Info(3)/=0 ) THEN
      ...
      END IF
    ELSE
      IF( (tn-Tout)*h<0._DP ) CYCLE integration_loop
      CALL DDATRP(tn,Tout,Y,Yprime,Neq,Iwork(LKOLD),Rwork(lphi),Rwork(LPSI))
      Idid = 3
      T = Tout
      EXIT integration_loop
