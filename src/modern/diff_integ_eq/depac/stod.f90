...
SUBROUTINE STOD(Neq,Y,Yh,Nyh,Yh1,Ewt,Savf,Acor,Wm,Iwm,F,JAC)
  ...
  REAL(SP) :: dcon, ddn, del, delp, dsm, dup, exdn, exsm, exup, r, rh, rhdn, &
    rhsm, rhup, told
        ...
        ELSE
          ...
          IF( m == 3 ) THEN
            ...
            ELSE
              ...
              EXIT corrector_outer
            END IF
          ELSEIF( m >= 2 .AND. del > 2._SP * delp ) THEN
            ! Diverging
            IF( ipup_com /= 0 ) THEN
              ipup_com = miter_com
              CYCLE corrector_outer
...
CONTAINS
  ...
  SUBROUTINE compute_rh_factors()
    ...
    IF( kflag_com == 0 .AND. rh < 1.1_SP .AND. .NOT. change_order ) THEN
      ialth_com = 3
      r = 1._SP / tesco_com(2, nqu_com)
      Acor(1:n_com) = Acor(1:n_com) * r
      do_exit = .TRUE.
