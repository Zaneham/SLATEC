!** STEPS
SUBROUTINE STEPS(DF,Neqn,Y,X,H,Eps,Wt,Start,Hold,K,Kold,Crash,Phi,P,Yp,&
  ...
  !   the function routine  D1MACH,  U = eps_sp, or by
  ...
  REAL(SP) :: absh, big, erk, erkm1, erkm2, erkp1, err, hnew, p5eps, r, reali, &
    realns, rho, round, tau, temp1, temp2, temp3, temp4, temp5, temp6, u
  LOGICAL :: step_successful, lower_order
  !
  ...
  REAL(SP), PARAMETER :: gstr(13) = [ 0.5_SP, 0.0833_SP, 0.0417_SP, 0.0264_SP, &
  ...
  IF( Start ) THEN
    ...
    u = eps_sp
    big = SQRT(huge_sp)
    CALL HSTRT(DF, Neqn, X, X+H, Y, Yp, Wt, 1, u, big, Phi(1,3), Phi(1,4), Phi(1,5), Phi(1,6), H)
    !
    Hold = 0._SP
    K = 1
    Kold = 0
  ...
  step_loop: DO
    ...
    ELSEIF( knew == km1 ) THEN
      ...
      lower_order = .TRUE.
    ELSEIF( kp1 > Ns ) THEN
      ! Step size not constant, keep current order
      lower_order = .FALSE.
    ELSE
      DO l = 1, Neqn
      ...
      IF( K > 1 ) THEN
        IF( erkm1 <= MIN(erk, erkp1) ) THEN
          lower_order = .TRUE.
        ELSEIF( erkp1 < erk .AND. K /= 12 ) THEN
          ! Raise order
          K = kp1
          erk = erkp1
        END IF
