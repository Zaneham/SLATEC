!** LSOD
SUBROUTINE LSOD(F,Neq,T,Y,Tout,Rtol,Atol,Idid,Ypout,Yh,Yh1,Ewt,Savf,&
    Acor,Wm,Iwm,JAC,Intout,Tstop,Tolfac,Delsgn)
  !> Subsidiary to DEBDF
  !***
  ! **Library:**   SLATEC
  !***
  ...
  ! **See also:**  DEBDF
  !***
  ! **Routines called:**  D1MACH, HSTRT, INTYD, STOD, VNWRMS, XERMSG
  !***
  ...
  LOGICAL :: store_and_return, rel_error_inappropriate
  ...
  tolerance_check: DO k = 1, Neq
    IF( nrtolp <= 0 ) THEN
      IF( Rtol(k) < 0. ) THEN
        ...
        Idid = -33
        IF( natolp > 0 ) EXIT tolerance_check
        nrtolp = 1
      ELSEIF( natolp > 0 ) THEN
        IF( itol_com == 0 ) EXIT tolerance_check
        CYCLE tolerance_check
      END IF
    END IF
    !
    IF( Atol(k) < 0. ) THEN
      ...
      ERROR STOP 'LSOD : IN DEBDF, THE ABSOLUTE ERROR TOLERANCES&
        & MUST BE NON-NEGATIVE. IN THE CASE OF VECTOR ERROR TOLERANCES,&
        & NO FURTHER CHECKING OF ATOL COMPONENTS IS DONE.'
      Idid = -33
      IF( nrtolp > 0 ) EXIT tolerance_check
      natolp = 1
    END IF
    IF( itol_com == 0 ) EXIT tolerance_check
  END DO tolerance_check
  !
  IF( itstop_com == 1 ) THEN
    IF( SIGN(1._SP, Tout-T) /= SIGN(1._SP, Tstop-T) .OR. ABS(Tout-T) > ABS(Tstop-T) ) THEN
      WRITE (xern3,'(1PE15.6)') Tout
      WRITE (xern4,'(1PE15.6)') Tstop
      ERROR STOP 'LSOD : IN DEBDF, YOU HAVE CALLED THE CODE WITH THAT VALUE OF&
  ...
  IF( init_com /= 0 ) THEN
    ...
    !
    IF( init_com /= 1 ) THEN
      IF( Delsgn * (Tout - T) < 0._SP ) THEN
        WRITE (xern3,'(1PE15.6)') Tout
        ERROR STOP 'LSOD : IN DEBDF, BY CALLING THE CODE WITH THAT VALUE OFTOUT&
  ...
  IF( init_com == 1 ) THEN
    ...
    DO l = 1, Neq
      ...
      IF( tol == 0._SP ) THEN
        rel_error_inappropriate = .TRUE.
        EXIT
      END IF
      Ewt(l) = tol
    END DO
    !
    IF( .NOT. rel_error_inappropriate ) THEN
      big = SQRT(huge_sp)
      CALL HSTRT(F, Neq, T, Tout, Y, Yh(1,2), Ewt, 1, uround_com, big, Yh(1,3), Yh(1,4), Yh(1,5), &
        Yh(1,6), h_com)
      !
  ...
  IF( rel_error_inappropriate ) THEN
  ...
  IF( .NOT. store_and_return .AND. .NOT. rel_error_inappropriate ) THEN
    !
    !              ......................................................
    !
    ...
    !                 RETURN
    !
    integration_loop: DO WHILE( ABS(tn_com - T) < absdel )
      !
      ...
      END IF
      !
      IF( iinteg_com /= 0 .AND. Intout ) THEN
        !
