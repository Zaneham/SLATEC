          ...
          ELSE
            ...
            ELSE
              ...
              IF( Key<0 .OR. Key>3 ) THEN
                ...
                RETURN
              ELSE
                IF( Key==0 .AND. Lw<5*M ) THEN
                  ERROR STOP 'DULSIA : INSUFFICIENT WORK SPACE'
                  Info = -1
                  RETURN
                END IF
                IF( Key==1 .AND. Lw<4*M ) THEN
                  ERROR STOP 'DULSIA : INSUFFICIENT WORK SPACE'
                  Info = -1
                  RETURN
                END IF
                IF( Key==2 .AND. Lw<4*M ) THEN
                  ERROR STOP 'DULSIA : INSUFFICIENT WORK SPACE'
                  Info = -1
                  RETURN
                END IF
                IF( Key==3 .AND. Lw<3*M ) THEN
                  ERROR STOP 'DULSIA : INSUFFICIENT WORK SPACE'
                  Info = -1
                  RETURN
                END IF
                ...
                ELSE
                  ...
                  IF( Key==1 ) THEN
                    !
                    IF( Ae(1)<0._DP ) THEN
                      ERROR STOP 'DULSIA : AE(I) < 0'
                      RETURN
                    END IF
                    DO i = 1, M
                      IF( Re(i)<0._DP ) THEN
                        ERROR STOP 'DULSIA : RE(I) < 0'
                        RETURN
                      END IF
                      IF( Re(i)>1._DP ) THEN
                        ERROR STOP 'DULSIA : RE(I) > 1'
                        RETURN
                      END IF
